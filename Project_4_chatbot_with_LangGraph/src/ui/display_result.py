            ...
            if self.user_message:
                ...
                with st.chat_message("assistant"):
                    with st.spinner("🔍 Generating response..."):
                        try:
                            graph_input = {"messages": [(msg["role"], msg["content"]) for msg in st.session_state["messages"]]}
                            result = self.graph.invoke(graph_input)

                            # ✅ FIX: safely extract last AI message
                            messages = result.get("messages", [])
                            response = messages[-1].content if messages else "⚠️ No response"
